...
const onFirstReady = (img: HTMLImageElement) => {
  ...
  if (canvas && img.naturalWidth && img.naturalHeight) {
    ...
    // Dimensões internas do canvas = resolução nativa das imagens
    // Não multiplicar pelo DPR aqui — as imagens já têm resolução própria
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const drawFrame = (index: number) => {
        ...
        ctx2.imageSmoothingQuality = "high";     // ← indentação correta
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse at 50% 35%, rgba(234,88,12,0.15) 0%, rgba(0,0,0,0) 65%)",
          }}
        />
        <div
          aria-hidden
          ...
          style={{
            background:
              "radial-gradient(ellipse at center, transparent 50%, rgba(10,10,10,0.85) 100%)",
          }}
        />

        <div className="relative z-10 flex h-full w-full flex-col items-center justify-center gap-1 md:gap-2">
          <h2
            ref={titleTopRef}
            aria-hidden
            className="font-black uppercase text-center pointer-events-none select-none text-orange-500"
            style={{
          ...
          <div
            ref={cardRef}
            className="relative overflow-hidden rounded-[12px] shadow-[0_20px_80px_rgba(0,0,0,0.7)] ring-1 ring-white/10 will-change-transform md:rounded-[16px]"
            style={{
...
style={{ display: "block", width: "100%", height: "100%" }}
          ...
          <h2
            ref={titleBottomRef}
            aria-hidden
            className="font-black uppercase text-center pointer-events-none select-none text-white"
            style={{
